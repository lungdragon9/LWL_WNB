...
public class LWL 
  ...
  public double[] distributionForInstance(Instance instance) throws Exception {
	...
	/**
	* Calculates the weights for each instances
	*
	* @param instance the instance to be classified
	* @return the weights for each instance
	* @author James Stamps
	* @throws Exception if distribution can't be computed successfully
	*/
	public double[] getWeightsForDistributionForInstance(Instance instance) throws Exception {
	    ...
	    System.out.println("LWL Neighbours : " + neighbours.numInstances());
		...
		double[] weights = new double[neighbours.numInstances()];
		// Pass the distances through a weighting kernel
		for (int i = 0; i < distances.length; i++) {
		  switch (m_WeightKernel) {
			case LINEAR:
